...
function encrypt(){
    ...
    for( let i = 0 ; i < plaintext.length ; i = i + 1 ){ // for each word in the plaintext, do the following
      if (plaintext[i].length > 2) { // If the current word is 2 or more letters, do the following:
        ...
        ciphertext += (plaintext[i]).replace(/a/g, "1").replace(/e/g, "5").replace(/i/g, "9").replace(/o/g, "16").replace(/u/g, "21"); // replace each vowel with the number placement in the alphabet eg.  e is the fifth letter of the alphabet
  ...
  function decrypt() {
    ...
    for( let i = 0 ; i < cipherList.length ; i = i + 1 ){ // for each word in the ciphertext, do the following
        if (cipherList[i].length > 2) { // If the current word is 2  or more letters, do the following:
            plaintext += (cipherList[i]).replace(/1/g, "a").replace(/5/g, "e").replace(/9/g, "i").replace(/"16"/g, "o").replace(/u/g, "21").replace(/bi/g, " "); // replace these numbers with the the special value 
